...
public interface EmpLazyDao {
	...
	@Results(id = "empMapper", value = { @Result(id = true, column = "empno", property = "empno"),
			@Result(column = "ename", property = "ename"), @Result(column = "esex", property = "esex"),
			@Result(column = "eage", property = "eage"), @Result(column = "esalary", property = "esalary"),
			@Result(column = "deptno", property = "dept", one = @One(select = "com.emp.dao.DeptDao.selectById", fetchType = FetchType.LAZY)),
			@Result(column = "mgrno", property = "mgr", one = @One(select = "com.emp.dao.EmpLazyDao.queryById", fetchType = FetchType.LAZY)) })
	...
	// 依据姓名模糊查询
	@Select("select empno,ename,esex,eage,esalary,deptno,mgrno from t_emp " + "where INSTR(ename,#{ename})>0")
	@ResultMap("empMapper")
	...
	@Update("update t_emp set ename=#{ename},esex=#{esex},eage=#{eage},esalary=#{esalary}, "
			+ "deptno=#{dept.deptno},mgrno=#{mgr.empno} " + "where empno=#{empno}")
	...
	// 查询出所有的经理
	@Select("select distinct m.empno,m.ename " + "from t_emp m inner join t_emp e " + "on m.empno=e.mgrno "
